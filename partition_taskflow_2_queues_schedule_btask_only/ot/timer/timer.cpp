...
void Timer::_get_topo_order() {
  ...
  */
  ...
  std::set<int> _bpartition_refined(_bpartition.begin(), _bpartition.end()); // _fpartition results looks weird?? need to be refined
...
void Timer::_build_partitioned_taskflow() {
  ...
  for(auto& pin_vec : _btopo_partitioned_pins) {
    ...
    auto start = std::chrono::steady_clock::now();
    btask[index] = _taskflow.emplace([this, pin_vec, scan, index, ready_queue, pending_queue, visited, pin_vec_map] () mutable{
          ...
          if(p->_ftask_status.compare_exchange_strong(cur_task_status, 1)) {
            ...
            for(auto arc : p->_fanin) {
              ...
              if(from->_has_state(Pin::BPROP_CAND)) {
                if(from->_bcur_num_dependents.fetch_sub(1) == 1) { // push all successors with no dependents after sub infrom ready queue
...
//                    std::cerr << "push from status 1: " << from->_name << "\n";
            ...
            for(auto arc : p->_fanin) {
              Pin* from = &(arc->_from);
              if(from->_has_state(Pin::BPROP_CAND)) {
                if(from->_bcur_num_dependents.load() == 0) { // push all successors with no dependents after sub infrom ready queue
                  if((from->_bpartition_id & scan) == scan && !visited[pin_vec_map[from]]) {
